class Traitor {
  constructor() {   // El constructor inicializa las propiedades del objeto 'Traitor'
...
// Método para hacer que el traidor aparezca en la pantalla
  aparecer() {
    this.aparecido = true; // Cuando se llame a este método, el traidor aparecerá // Cambia el estado de 'aparecido' a 'true', lo que permite que el traidor se mueva y se muestre
  }
...
// Método para mover al traidor
  move() {
    if (this.aparecido) {  // Solo mueve al traidor si ha aparecido
      ...
      if (this.x > width || this.x < 0) this.speed *= -1;  // Invierte la dirección del movimiento (si va a la derecha, irá a la izquierda y viceversa)
    }
  }
  
// Método para dibujar al traidor en la pantalla
  display() {
