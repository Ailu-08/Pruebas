...
updateProjectiles() { 
  // Actualizar y mostrar proyectiles de Dastan
    for (let i = this.dastanProjectiles.length - 1; i >= 0; i--) {
      ...
      if (this.traitor.aparecido && projectile.hits(this.traitor)) { //Si el traidor ha aparecido y el proyectil golpea al triador: 
    ...
    for (let i = this.traitorProjectiles.length - 1; i >= 0; i--) {
      ...
      if (projectile.hits(this.dastan)) {
        this.dastanLives--;
  ...
  for (let i = this.secuaces.length - 1; i >= 0; i--) {
    ...
    for (let j = this.dastanProjectiles.length - 1; j >= 0; j--) {
      ...
      if (projectile.hits(secuaz)) { //Si el proyectil golpea al Secuaz
        this.secuaces.splice(i, 1); // Elimina al secuaz
